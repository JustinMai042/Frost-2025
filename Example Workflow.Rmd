...
```{r}
set.seed(123)
openml_datasets_trimmed <- purrr::imap(openml_datasets_list, ~ {
  if (nrow(.x) > 5000) {
    dplyr::slice_sample(.x, n = 5000)
  } else {
    .x
  }
})
...
df <- openml_datasets_trimmed[[1]]
  ...
  rec <- recipe(target ~ ., data = train_full) |>
    step_rm(all_predictors(), -all_numeric()) |>
    step_dummy(all_nominal(), one_hot = TRUE) |>
    step_zv(all_predictors()) |>
    step_corr(all_numeric(), threshold = 0.9) |>
...
openml_datasets_trimmed <- openml_datasets_trimmed[1:5]
